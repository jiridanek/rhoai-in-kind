...
import (
	...
	appsv1 "api-extension/pkg/apis/user/v1"
...
func (r *REST) Get(ctx context.Context, name string, options *metav1.GetOptions) (runtime.Object, error) {
	...
	if !ok {
		// Handle error: User information is not available in the context
		// This might indicate an issue with authentication middleware
		klog.Errorf("User information not found in context for resource %s", name)
		// Returning an error might be appropriate here, depending on your requirements
		// For example: return nil, apierrors.NewInternalError(fmt.Errorf("user info not found in context"))
		// Or proceed with a default/anonymous user if applicable
		return nil, fmt.Errorf("user information not found in context")
	}

	// Get the username
	// NOTE: In Kubernetes, the 'name' parameter in the Get request often refers to the
	// resource name being requested, not necessarily the username making the request.
	// If you specifically want the *requesting* user's details, use userInfo.GetName().
	// If the intent is to return a "User" resource *named* after the requesting user,
	// then using userInfo.GetName() might be correct. Let's assume that's the goal here.

	requestingUsername := userInfo.GetName()
	parts := strings.Split(requestingUsername, ":")
	requestingUsername = parts[len(parts)-1]
	klog.V(4).Infof("Request received from user: %s", requestingUsername)
...
//	return nil, fmt.Errorf("no list, bro %+v", options)
//}
...
func (r *REST) buildTableFromApplications(apps []appsv1.User) metav1.Table {
	table := metav1.Table{
...
func (r *REST) buildTableFromApplication(app appsv1.User) metav1.Table {
	table := metav1.Table{
		ColumnDefinitions: []metav1.TableColumnDefinition{
			{Name: "NAME", Type: "string", Description: "Name of the Application", Priority: 0},
			{Name: "READY", Type: "string", Description: "Ready status of the Application", Priority: 0},
